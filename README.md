...
#### 4. **customizations** (Add-ons/Toppings)
```typescript
{
  $id: string;
  name: string;             // Customization name (e.g., "Extra Cheese")
  type: 'topping' | 'side' | 'sauce';
  price: number;            // Additional price
  description?: string;
}
```

#### 5. **menu_customizations** (Junction Table)
```typescript
{
  $id: string;
  menuId: string;           // Reference to menu
  customizationId: string;  // Reference to customizations
}
```
...
**Xem chi tiết**: `app-web/docs/ERD_DATABASE_SCHEMA.md`
...
**Xem chi tiết**: `app-web/lib/appwrite.ts` và `admin/src/lib/api.ts`
...
**Hoặc kết nối repository với Vercel/Netlify để auto-deploy.**
